...
trait WaitUtils extends LazyLogging {
  ...
  def waitFor[F[_]](maxDuration: FiniteDuration, delay: FiniteDuration = 500.microseconds)(
    action: => Boolean
  )(implicit F: Sync[F], T: Timer[F]): F[Boolean] = {
      ...
      F.delay {
        ...
        case false =>
          F.delay(
            ...
            T.sleep(delay) *> loop(spent + delay, maxDuration)
        case _ =>
